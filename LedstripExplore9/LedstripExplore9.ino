...
void loop() {
  ...
  // Sequentially turn on each LED to red
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[i] = CRGB(255, 0, 0);
    FastLED.show();
    delay(50); // Delay for a smooth transition
  }
  delay(1000); // Wait for a second after turning all LEDs red

  // Sequentially turn on each LED to green
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[i] = CRGB(0, 255, 0);
    FastLED.show();
    delay(50); // Delay for a smooth transition
  }
  delay(1000); // Wait for a second after turning all LEDs green

  // Sequentially turn on each LED to blue
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[i] = CRGB(0, 0, 255);
    FastLED.show();
    delay(50); // Delay for a smooth transition
  }
  delay(1000); // Wait for a second after turning all LEDs blue
}
